...
const Workerhistory=()=> {
 ...
 var unsubscribe = async (id)=>{
  ...
  try{
    ...
    axios.delete(URL).then((res)=>{refresh()}).catch(err => console.log(err))
        ...
        <View style={styles.user}>
            <View style={styles.userrrr}>
                ...
                <View style={styles.userr}>
                    ...
                    <AirbnbRating style={styles.star} count={1} size={30} showRating={false} startingValue={1}  ratingColor='#f94368' ratingBackgroundColor='#f9b313' type='custom' />
            </View>
        </View>
        <ScrollView style={styles.userhis}  vertical={true} >
       { events.map((ele,i)=><View style={styles.userhiss}>
              <View style={styles.alloff1}>
                ...
                <View style={styles.userrhis}>
                  <Text> {ele.label}</Text>
                  <Image style={styles.imgggg} source={{ uri: ele.imageUrl }}></Image>
                  <TouchableWithoutFeedback  onPress={()=>unsubscribe(ele.eventID)}>         
                  <Image style={styles.imggg} source={{ uri: "https://pngset.com/images/cancel-icon-first-aid-symbol-text-logo-transparent-png-1419157.png" }}></Image> 
